<template>
  <div class="fixed inset-0 flex items-center justify-center bg-amber-75 text-red-950 dark:text-white dark:bg-black">
    <div
          ...
          <template #default>
            <div v-if="showData" class="flex flex-col ">
              ...
              <div class="flex gap-20 my-10 justify-center">
                <span class="text-[1.5rem] break-all md:px-50 px-20">{{ showData.quesConfig.title }}{{
                  showData.quesConfig.status == 1 ? "[草稿预览]" : "test" }}</span>
              </div>
